...
function ClerkDashboard({ user, onLogout }) {
  ...
  const handleViewAppStatement = async (app) => {
  ...
  const fetchLatest = async () => {
    try {
      const [statRes, schRes, viewRes, studRes, docRes, appRes] = await Promise.all([
  ...
  const handleUpdateAppStatus = async (appId, status) => {
    ...
    try {
      await axios.post(`${API}/admin/applications/${appId}/status`, { status });
      fetchLatest();
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col relative">
        ...
        <div className="flex flex-wrap gap-2 mb-8 bg-white p-1.5 rounded-xl w-fit border border-slate-200 shadow-sm">
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              ...
              {tab.id === "applications" && applications.filter(a => a.status === 'applied').length > 0 && (
              ...
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                  <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                    <ClipboardList className="h-5 w-5 text-indigo-500" /> Scholarship Applications
                    <span className="bg-indigo-100 text-indigo-700 text-xs font-black px-2.5 py-1 rounded-full ml-2">{applications.length} Total</span>
                    {applications.filter(a => a.status === 'applied').length > 0 && (
                      <span className="bg-amber-100 text-amber-700 text-xs font-black px-2.5 py-1 rounded-full flex items-center gap-1">
                        <Bell className="h-3 w-3" /> {applications.filter(a => a.status === 'applied').length} New
                      </span>
                    )}
                  </h3>
                            ...
                            <td className="p-4">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded ${
                                  a.verification_status === 'verified' ? 'bg-emerald-100 text-emerald-700' :
                                  a.verification_status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
                                }`}>
                                  {a.verification_status}
                                </span>
                              </div>
                            </td>
                            <td className="p-4 text-slate-600 text-sm font-medium">{a.college_name || '-'}</td>
                            <td className="p-4">
                              <p className="font-bold text-indigo-700 text-sm">{a.scholarship_name}</p>
                              <p className="text-xs text-slate-400">{a.provider}</p>
                            </td>
                            <td className="p-4">
                            ...
                            <td className="p-4 text-right pr-6">
                              <div className="flex items-center justify-end gap-2">
                                {a.personal_statement && (
                                  <button
                                    onClick={() => handleViewAppStatement(a)}
                                ...
                                {a.scholarship_link && (
                                  <a href={a.scholarship_link} target="_blank" rel="noreferrer"
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                      <PieChart className="h-5 w-5 text-indigo-500" /> Scholarship Applications Report
                    </h3>
                    <button onClick={fetchLatest} className="text-slate-500 hover:text-emerald-600"><RefreshCw className="h-4 w-4" /></button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div className="bg-indigo-50 rounded-xl p-6 border border-indigo-100">
                      <p className="text-sm font-bold text-indigo-700 uppercase tracking-widest mb-2">Total Applications Received</p>
                      <p className="text-4xl font-black text-indigo-900">{reportData.summary.total_applications}</p>
                    </div>
                    <div className="bg-emerald-50 rounded-xl p-6 border border-emerald-100">
                      <p className="text-sm font-bold text-emerald-700 uppercase tracking-widest mb-2">Est. Scholarship Amount</p>
                      <p className="text-4xl font-black text-emerald-900">₹{reportData.summary.total_amount.toLocaleString()}</p>
                    </div>
                  </div>

                  <h4 className="font-bold text-slate-700 mb-4 border-b border-slate-100 pb-2">Breakdown by Category</h4>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs font-bold uppercase tracking-widest">
                          ...
                          <th className="p-4 text-right pr-6">Est. Amount</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(reportData.report).map(([type, data]) => (
                          <tr key={type} className="border-b border-slate-100 hover:bg-slate-50">
                            <td className="p-4 pl-6 font-bold text-slate-800">{type}</td>
                            <td className="p-4"><span className="bg-indigo-100 text-indigo-700 px-2.5 py-1 rounded-full text-xs font-black">{data.count}</span></td>
                            <td className="p-4"><span className="bg-emerald-100 text-emerald-700 px-2.5 py-1 rounded-full text-xs font-black">{data.approved_count}</span></td>
                            <td className="p-4 text-right pr-6 font-bold text-emerald-700">₹{data.estimated_amount.toLocaleString()}</td>
                          </tr>
                        ))}
                        {Object.keys(reportData.report).length === 0 && (
                          <tr>
                            <td colSpan="4" className="p-8 text-center text-slate-400 font-bold">No application data available yet.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                        ...
                        <div className="space-y-3">
                          {selectedStudent.applications.map(app => (
                            <div key={app.id} className="border border-slate-200 rounded-lg p-3 bg-white shadow-sm">
                               <div className="flex justify-between items-start mb-2">
                                 <div>
                                   <p className="font-bold text-indigo-700 text-sm mb-0.5">{app.scholarship_name}</p>
                                   <p className="text-[10px] text-slate-500 font-medium">{app.provider}</p>
                                 </div>
                                 <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full border ${
                                   app.status === 'approved' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' :
                                   app.status === 'rejected' ? 'bg-red-100 text-red-700 border-red-200' : 'bg-amber-100 text-amber-700 border-amber-200'
                                 }`}>{app.status}</span>
                               </div>
                               ...
                               {app.status === 'applied' && (
                                 <div className="flex gap-2 mt-3 pt-3 border-t border-slate-50">
                                   <button 
                                     ...
                                     disabled={selectedStudent.student.verification_status !== 'verified'}
                                     className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${
                                       selectedStudent.student.verification_status === 'verified'
      ...
      {/* Personal Statement Viewer Modal */}
      {selectedAppStatement && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-200">
            <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex justify-between items-center rounded-t-2xl">
              <h2 className="text-lg font-black text-slate-800 flex items-center gap-2">
                <FileText className="h-5 w-5 text-indigo-500" /> Personal Statement
              </h2>
              <button onClick={() => { setSelectedAppStatement(null); setSelectedAppDocs(null); }} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
                <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-1">Applicant</p>
                <p className="font-bold text-slate-800">{selectedAppStatement.student_name}</p>
                <p className="text-xs text-slate-500">{selectedAppStatement.student_email} · {selectedAppStatement.college_name || 'N/A'}</p>
              </div>
              <div className="bg-slate-50 border border-slate-100 rounded-xl p-4">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Scholarship</p>
                <p className="font-bold text-indigo-700">{selectedAppStatement.scholarship_name}</p>
              </div>
              <div className="bg-white border border-slate-200 rounded-xl p-4">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Statement</p>
                <p className="text-sm text-slate-700 font-medium whitespace-pre-wrap leading-relaxed">
                  {selectedAppStatement.personal_statement || <span className="text-slate-400 italic">No statement provided.</span>}
                </p>
              </div>
              <div className="bg-white border border-slate-200 rounded-xl p-4">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Uploaded Documents</p>
                {selectedAppDocs === null ? (
                  <div className="flex justify-center py-4"><RefreshCw className="h-5 w-5 text-indigo-500 animate-spin" /></div>
                ) : selectedAppDocs.length === 0 ? (
                  <p className="text-sm text-slate-400 italic">No documents uploaded.</p>
                ) : (
                  <div className="space-y-2">
                    {selectedAppDocs.map(doc => (
                      <div key={doc.id} className="border border-slate-100 rounded-lg p-2.5 hover:bg-slate-50 transition-colors flex justify-between items-center">
                         <div className="flex flex-col">
                           <a href={`${BACKEND_URL}/${doc.file_path}`} target="_blank" rel="noreferrer" className="text-sm font-bold text-indigo-600 hover:underline flex items-center gap-1">
                             <FileText className="h-3 w-3" /> {doc.document_type.replace('_', ' ')}
                           </a>
                           <span className="text-xs text-slate-500 mt-0.5">{doc.file_name}</span>
                         </div>
